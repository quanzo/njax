...
Краткий навигационный индекс по всем PHPUnit-тестам проекта. Помогает быстро найти: «какой тест проверяет сценарий X?» и «как запустить только нужный набор?».
...
- Клиент [`src/client/TaskManager.js`](../src/client/TaskManager.js).
